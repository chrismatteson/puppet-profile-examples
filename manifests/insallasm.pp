# == Class: ora_install::installasm
#
#
define ora_install::installasm(
  $version                   = undef,
  $file                      = undef,
...
)
{
  ...
  #notify {"ora_install::installasm file without extension ${$file_without_ext} ":}
  ...
  if $found == undef {
    ...
    if ( $found ) {
      ...
      notify {"ora_install::installasm ${grid_home} does not exists":}
      $continue = true
    }
  }
  ...
  if ( $continue ) {
    ...
    if $puppet_download_mnt_point == undef {
      $mountPoint     = 'puppet:///modules/ora_install/'
    } else {
    ...
    ora_install::utils::dborainst{"grid orainst ${version}":
      ora_inventory_dir => $oraInventory,
      os_group          => $group_install,
    }
    ...
    if ! defined(File["${download_dir}/grid_install_${version}.rsp"]) {
      file { "${download_dir}/grid_install_${version}.rsp":
        ...
        content => template("ora_install/grid_install_${version}.rsp.erb"),
        mode    => '0770',
        owner   => $user,
        group   => $group,
        require => [Ora_install::Utils::Dborainst["grid orainst ${version}"],
                    Db_directory_structure["grid structure ${version}"],],
      }
    }
    ...
    if ! defined(File["${user_base_dir}/${user}/.bash_profile"]) {
      file { "${user_base_dir}/${user}/.bash_profile":
        ...
        # content => template('ora_install/grid_bash_profile.erb'),
        content => regsubst(template('ora_install/grid_bash_profile.erb'), '\r\n', "\n", 'EMG'),
        mode    => '0775',
        owner   => $user,
        group   => $group,
      }
    ...
    if ($::osfamily == 'RedHat') and ($::operatingsystemmajrelease == '7')
    {
      file {'/etc/systemd/system/ohas.service':
        ...
        content => template('ora_install/ohas.service.erb'),
        mode    => '0644',
    ...
    if ( $grid_type == 'CRS_SWONLY' ) {
      ...
      file { "${download_dir}/cfgrsp.properties":
        ...
        content => template('ora_install/grid_password.properties.erb'),
        mode    => '0600',
        owner   => $user,
        group   => $group,
        require => [Exec["run root.sh grid script ${title}"],
